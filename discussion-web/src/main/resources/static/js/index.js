	...
	function lockPage(){
		layer.open({
			...
			content : '	<div class="admin-header-lock" id="lock-box">'+
							...
							'<div class="admin-header-lock-name" id="lockUserName">北京律典通</div>'+
							'<div class="input_btn">'+
		...
		for(var i=0;i<menu.length;i++){
			...
			if(menu[i].icon){
				if(menu[i].icon.split("-")[0] == 'icon'){
					openTitle += '<span><i class="iconfont '+menu[i].icon+'"></i></span>';
				}else{
					openTitle += '<span><i class="layui-icon">'+menu[i].icon+'</i></span>';
				}
			}
			openTitle += '<span><cite>'+menu[i].title+'</cite></span>';
			openTitle += '<span class="ccck"><i class="layui-icon layui-unselect layui-tab-close" data-id="'+menu[i].layId+'">&#x1006;</i></span>';
