...
var p = layui.define(["layer","jquery","element"],function(exports){
    ...
    var topMenusUrl = '/admin/system/user/getUserMenuAndTopMenus';
    $.get(topMenusUrl,function (data) {
        var ulTopMenusHtml = '';
        for(var i=0;i<data.length;i++){
            // console.log(data[i].title);
            ulTopMenusHtml += "<li class='layui-nav-item'><a class='onclickfunction' href='javascript:' onclick='getSubMenus("+data[i].id+")'><i class='layui-icon' style='font-size: 20px' data-icon='"+data[i].icon+"'>"+data[i].icon+"</i><cite>"+data[i].title+"</cite></a></li>"
        }
        // console.log(ulTopMenusHtml);
        $('#top_menus').html(ulTopMenusHtml);
    });

	//获取二级菜单数据
	Tab.prototype.render = function(pid) {
    	var url='';
    	if(pid == null || pid == undefined || pid == ''){
            url = this.tabConfig.url+"?pid=1";
		}else{
            url = this.tabConfig.url+"?pid="+pid;
		}
		...
		if(_this.attr("target") == "_blank"){
			...
			if(_this.find("i.iconfont,i.layui-icon").attr("data-icon") != undefined){
				if(_this.find("i.iconfont").attr("data-icon") != undefined){
					title += '<span><i class="iconfont '+_this.find("i.iconfont").attr("data-icon")+'"></i></span>';
				}else{
					title += '<span><i class="layui-icon">'+_this.find("i.layui-icon").attr("data-icon")+'</i></span>';
				}
			}
			//已打开的窗口中不存在
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				...
				tabIdIndex++;
				title += '<span><cite>'+_this.find("cite").text()+'</cite></span>';
				title += '<span class="ccck"><i class="layui-icon layui-unselect layui-tab-close" data-id="'+tabIdIndex+'">&#x1006;</i></span>';
	...
	//删除tab
	$("body").on("click",".top_tab li a span i.layui-tab-close",function(){
	//删除tab后重置session中的menu和curmenu
		liIndex = $(this).parent().parent().parent("li").index();
        var menu = JSON.parse(window.sessionStorage.getItem("menu"));
		...
		if(JSON.stringify(curmenu) != JSON.stringify(menu[liIndex-1])){  //如果删除的不是当前选中的tab
			...
			if($(this).parent().parent().parent("li").length > liIndex){
		...
		element.tabDelete("bodyTab",$(this).parent().parent().parent("li").attr("lay-id")).init();
		new Tab().tabMove();
	});
...
function getSubMenus(pid){
    var url='';
    if(pid == null || pid == undefined || pid == ''){
        url = "/admin/system/user/getUserMenu?pid=1";
    }else{
        url = "/admin/system/user/getUserMenu?pid="+pid;
    }
    var $= layui.jquery;
    $.get(url,function(data){
        //显示左侧菜单
