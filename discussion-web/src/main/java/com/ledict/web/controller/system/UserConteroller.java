...
import com.ledict.core.util.ToolUtil;
...
public class UserConteroller extends BaseController {
    ...
    @ResponseBody
    public List<ShowMenu> getUserMenu(String pid){
        Long userId = MySysUser.id();
        List<ShowMenu> list = menuService.getShowMenuByUser(userId,pid);
        return list;
    }
     ...
     * 获取用户所拥有的顶级菜单列表
     */
    @GetMapping("getUserMenuAndTopMenus")
    @ResponseBody
    public List<ShowMenu> getUserMenuAndTopMenus(){
        Long userId = MySysUser.id();
        List<ShowMenu> list = menuService.getShowMenuByUserAndTopMenu(userId);
        return list;
    }
