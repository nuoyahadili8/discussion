...
import com.ledict.core.service.MenuService;
import com.ledict.dao.MenuDao;
...
//    @Cacheable(value = "allMenus",key = "'user_menu_'+T(String).valueOf(#id)",unless = "#result == null or #result.size() == 0")
    @Override
    public List<ShowMenu> getShowMenuByUser(Long id,String pid) {
        Map<String,Object> map = Maps.newHashMap();
        map.put("userId",id);
        map.put("parentId",Integer.parseInt(pid));
        return baseMapper.selectShowMenuByUser(map);
    }

//    @Cacheable(value = "topMenus",key = "'user_topmenu_'+T(String).valueOf(#id)",unless = "#result == null or #result.size() == 0")
    @Override
    public List<ShowMenu> getShowMenuByUserAndTopMenu(Long id){
        Map<String,Object> map = Maps.newHashMap();
        map.put("userId",id);
        map.put("parentId",null);
        return baseMapper.selectShowMenuByUserAndTopMenu(map);
    }
